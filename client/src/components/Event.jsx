...
export default function Event({ 
  ...
  const handleOpenModal = (event) => {
    ...
    setUserType('student');
    // Panatilihin o i-prefill kung may nakalogin
    if (currentUser) {
      setRegistrationData(prev => ({
        ...prev,
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    // 1. Prepare the data payload (ISINAMA NA ANG EVENT DETAILS DITO)
    const payload = {
      eventId: selectedEvent.id || selectedEvent._id,
      eventTitle: selectedEvent.title,                            // <--- Idinagdag
      eventDate: selectedEvent.date,                              // <--- Idinagdag
      eventVenue: selectedEvent.venue || selectedEvent.location,  // <--- Idinagdag
      userType: userType,
    ...
    try {
      // 2. Send the POST request to your backend endpoint
